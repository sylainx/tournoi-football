<footer id="p">
    <div class="container-footer">
        <div id="content-footer1">
            <div class="content-text1">
                <img src="" alt="logo Inuka" id="logo-footer">
                <p></p>
                <p>
                    <i class="fas fa-map-marker-alt"> </i>
                    Tabarre 43, Blvd 15 Octobre<br/>
                    Port-au-prince, Haiti
                </p>
                <div class="phone-number row">
                <p>
                        <i class="fas fa-envelope"> </i>
                            info@splashteam
                        </p>
                   
                </div>
                    <!--<div class="mail-schedule row">
                        <p>
                        <i class="fas fa-envelope"> </i>
                            info@splashteam
                        </p>
                        <p>
                        
                    </div>-->
                    
                
            </div>
            <div class="content-text2">
                <h3>QUICK LINKS</h3>
                <p> <a href="index.php">Acceuil</a></p>
                <p> <a href="calendrier.php">Calendrier</a></p>
                <p><a href="statistics.php">Stat|Team</a></p>
                <p>unavalaible</p>
             </div>
             
            <div class="content-text3">
                <h3>SIGN UP FOR EMAIL ALERTS</h3>
                <p>select topics and stay current with our latest news.</p>
                <span class="email-input row"> 
                    <input class="footerMail" type="text" placeholder="Your email address">
                    <button class="btn-footer" type="submit" >SUBMIT</button>
                </span>
            </div>
        </div>
        <div class="copyright-items">
            <div class="copyright">  Tous droits réservés Université INUKA ©2021 </div>
                <div class="social-links">
                    <a href="#" id="animation" ><i class="fab fa-facebook-f"></i></a> 
                    <a href="#" id="animation" ><i class="fab fa-twitter"></i></a> 
                    <a href="#" id="animation"><i class="fab fa-instagram"></i></a>
                    <a href="#" id="animation"><i class="fab fa-youtube"></i></a>
                </div>  
                <div class="designer">Proudly designed and developed by us(Ludens509,GS7)</div>
        </div>
    </div>
     
    </footer>



